...
import { artistsPageContext,
    titleNameContext,
    titleNameContextProvider } from "./LayoutContainer"
...
const Header = (
  {titleName, cover, coverArt, alt}
    ...
    let {title, setTitle} = useContext(titleNameContext)
    let x = useContext(titleNameContext)
    function upTitle (val) {
      const x {title, setTitle} = useContext(titleNameContext)
      setTitle (val)
      alert('parm =', val)
      alert ('upTitle', title)
      console.log('x', x)
