...
const Layout = ({appContext}) => {
  ...
  const artistList = artists.filter(artists => artists.titleName === context[0].title)
   ...
   // below useRef is for use with tracking currently playing song,in order
   // to pause it when another is started.  Follow the props.

  const playerRef = useRef('default');

  console.log ("Layout", appContext)
