...
function stopOtherPlayers (currentPlayer) {
    console.log('1st current=',currentPlayer, 'other', otherPlayer)
    if (otherPlayer.length > 0 ) {
        otherPlayer[0].current.audio.current.pause()
    }
    otherPlayer.push(currentPlayer)
    // currentPlayer.current.audio.current.pause()
    console.log('2nd current=',currentPlayer, 'other', otherPlayer)
}
